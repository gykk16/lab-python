...
모집단이 어떤 부포든지 상관없이, 표본의 크기가 충분히 크다면
 모든 가능한 표본 평균은 모평균 주위에서 정규 분포를 따른다.

만약 모집단의 평균이 mu 이고, 표준편차가 sigma 인 정규 분포를 따른다면,
 표본 평균의 분포는 평균이 mu 이고, 분산이 sigma/sqrt(n) 인 정규분포.


베르누이 확률 변수(Bernoulli random variable):
   어떤 시행의 결과가 '성공', '실패', 중 하나로 나타나고,
   성공의 확률이 p, 실패할 확률이 1 - p 라 할때
   그 결과가 성공이면 확률 변수를 1을 갖고,
   결과가 실패면 확률 변수는 0을 갖는 확률 변수 X

베르누이 확률 질량 함수(PMF: Probability Mass Function)
   pmg(x) = p if x = 1, 1 - p if x = 0
          = (p ** x) * ((1 - p) ** (1 - x))

이항 확률 변수(binomial random variable):
   n 개의 독립적인 베르누이 확률 변수들 더한 것

(비유)
베르누이 확률 변수 -> 동전 한 개를 던질 때 앞면의 수
이항 확률 변수 -> 동전 n 개를 던질 때 앞면의 수

베르누이 확률 변수의 기대값(평균) = p, 표준편차 = sqrt(p(1 - p))
중심 극한 정리: n이 적당히 크다면, 이항 확률 변수는 대략
 평균이 np 이고 , 표준편차가 sqrt(np(1 - p))인 정규 분포의 확률과 같다.

-> 표본(sample)의 평균(np)와 분산(p(1 - p))를 예측할 수 있다.
...
from scratch06.e06_연속확률분포 import normal_cdf, normal_pdf
...
if __name__ == '__main__':
    ...
    # 이항 확률 변수의 분포는 n이 충분이 크면 정규 분포가 된다.
    mu = n * p  # 평균
    sigma = math.sqrt(n * p * (1 - p))  # 표준편차
    # 정규 분포 글기 위해
    x_line = range(min(data), max(data) + 1)    # range(0, n + 1)
    y_line = [normal_cdf(x, mu, sigma) for x in x_line]
