...
if __name__ == '__main__':
    # 1. 데이터 준비
    # 2. csv 파일에 컬럼 이름들이 없이 때문에 컬럼 이름 준비
    col_names = ['sepal-length', 'sepal-width',
                 'petal-length', 'petal-width',
                 'Class']

    # csv 파일에서 DataFrame 을 생성
    dataset = pd.read_csv('iris.csv',
                          header = None,
                          names = col_names)

    # DataFrame 확인
    print(dataset.shape)  # (row 개수, col 개수)
    print(dataset.info())  # 데이터 타입, row 개수, col 개수, 컬럼 데이터 타입
    ...
    # X = 전체 행, 마지막 열 제외한 모든 열 데이터
    X = dataset.iloc[:, : -1].to_numpy()  # DataFrame 을 np.ndarray 로 변환
    # print(X)
    ...
    X_train = scaler.transform(X_train)  # 스케일을(표준화) 수행
