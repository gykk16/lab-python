...
from scratch12.e04_NB함수제작 import calculate_class_probability, summarize_by_class
...
def train_test_split(df, test_size):
    ...
    """
    # DataFrame 을 numpy.ndarray 타입으로 변환
    array = df.to_numpy()
    # array 의 순서를 무작위로 섞음.
    np.random.seed(1213)
...
def predict(summaries, X_test):
    ...
    for test in X_test:
        ...
        # sorted(dict): dict 의 키들을 오름차순 정렬한 리스트
        # sorted(dict.values()): dict 의 값들을 오름차순 정렬한 리스트
        # sorted(dict.items()): (key, value) 튜플을 key 값을 기준으로 정렬
        # best_label, best_prob = None, -1
        # for k, v in probabilities.items():
        #     if v > best_prob:  # 더 큰 확률값을 찾을 경우
        #         best_prob = v  # 찾은 확률값으로 best_prob 을 업데이트
...
if __name__ == '__main__':
    ...
    # print(iris_dataset.iloc[:, -1])  # DataFrame 의 가장 마지막 컬럼
    iris_dataset.info()
    ...
    model = summarize_by_class(iris_train)  # GaussianNB 에서 사용할 mu, sigma
    print(model)
    print(model[0.0])
    ...
    df = cancer_dataset[cols]
    print(df.head())

    # print(cancer_dataset.loc[:, ::-1].head())   # 좌우반전
