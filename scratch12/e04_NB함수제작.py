...
def separate_by_class(dataset):
    ...
    separated = dict()  # 빈 dict 를 생성
    for i in range(len(dataset)):  # dataset 의 길이(원소의 개수) 만큼 반복
        vector = dataset[i]  # dataset 의 i 번째 row(원소)
        class_value = vector[-1]  # 벡터의 가장 마지막 원소가 레이블
        if class_value not in separated:  # 클래스 값이 dict 의 키로 존재하지 않으면
            separated[class_value] = []  # 비어 있는 리스트 생성
...
def separate_by_class2(dataset):
    ...
    for i in range(len(dataset)):  # 리스트의 원소 개수 만큼 반복
        vector = dataset[i]  # 리스트의 i 번째 원소
        class_value = vector[-1]  # 리스트의 마지막 원소는 클래스(레이블)
        # 클래스를 key 를 갖는 리스트에 vector 를 추가
        separated[class_value].append(vector)
...
def summarize_dataset(dataset):
    ...
    # 마지막 컬럼은 데이터가 아니라 클래스(레이블) 이므로 평균 표쥰편차가 필요 없음
...
def summarize_by_class(dataset):
    ''' 데이터 세트의 컬럼(변수, 특성)들에 대해서, 각 클래스 별로 요약
        평균, 표준편차, 개수 요약
    ...
    summaries = dict()
    for class_value, vectors in separated.items():  # dict 의 원소 뽑을때 .keys, .values, .items
