...
데이터 과학을 하다보면, 최적환 문제들을 만나게 됨.
최적화 문제 - 특정 상황에서 가장 적합한 모델을 찾는 문제

(예)
모델의 오류(error)를 "최소화", liklihood(우도)를 "최대화"
타겟 함수를 최소(혹은 최대)로 만들어 주는 파라미터를 탖는 문제

곡선의 접선을 탖고, 접선의 기울기 방향으로 점을 이동시켜 나가면서 최소(최대)값을 찾음
...
def move(x, direction, step = -0.1):
    ''' x 좌표를 새로운 x 로 이동
        direction: 접선의 기울기
        step > 0 : 접선과 같은 방향으로 이동 -> 최대값 찾기
        step < 0 : 접선과 반대 방향으로 이동 -> 최소값 찾기
        '''
...
if __name__ == '__main__':
    ...
    # x ** 2 그래프에서 x = -1 에서의 접선을 그리기 위해
        # 직선의 방정식 y = ax + b 에서 기울기 a와 절편 b를 찾아야함
        # 기울기 a는 x ** 2 의 미분 값으로 찾음
        # y절편은 (x1, f(x1))을 지나는 직선임을 이용해서 찾음
    a = f_derivative(-1)    # x = -1 에서 접선의 기울기
    x1, y1 = -1, f(-1)  # x = -1에서 곡선과 접선이 만나는 점의 좌표
    tangents = [tangent(x, a, x1, y1) for x in xs]

    # 도함수의 근사값을 사용해서 x = -1 에서 기울기를 찾음
    # h 값이 0에 가까울수록 더 정확한 접선의 기울기가 됨
    h = 0.1
    ...
    # 실제 기울기(f_derivative)와 기울기 근사값(difference_quotinet) 비교
    ...
    plt.scatter(xs, actuals, label = 'actual', marker = 'o')
    plt.scatter(xs, estimates_1, label = 'h = 1', marker = '+')
    plt.scatter(xs, estimates_2, label = 'h = 0.1', marker = 'x')
    plt.legend()
    plt.show()
    ...
    for _ in range(5):
        ...
        print('ttttttttt', tangent_estimates)
